...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        # Total de lanzamientos exitosos por sitio (class=1)
        ...
        # Éxitos vs fallos para el sitio seleccionado
        df_site = spacex_df[spacex_df['Launch Site'] == entered_site]
        fig = px.pie(
            df_site,
            names='class',  # cuenta categorías 0/1
...
def update_scatter(selected_site, payload_range):
    ...
    # Filtrar por sitio si aplica
    if selected_site != 'ALL':
        df_filtered = df_filtered[df_filtered['Launch Site'] == selected_site]
        title = f'Correlation between Payload and Success — {selected_site}'
    else:
